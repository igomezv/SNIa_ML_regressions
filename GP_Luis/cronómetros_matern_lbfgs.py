# -*- coding: utf-8 -*-
"""Cronómetros matern lbfgs.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CkWPrQmAAo9vbo1FmNHU0-9U1ofEXI4G
"""
...
data = pd.read_csv("/content/drive/MyDrive/Astrofinal/Hz31.txt", sep=" ", header=0)
redshift = torch.tensor(data['z'].values, dtype=torch.float)
mag = torch.tensor(data['h(0)'].values, dtype=torch.float)
mag_err = torch.tensor(data['err'].values, dtype=torch.float)
...
class GaussianProcess(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(nu=2.5, lengthscale=1))
...
plt.xlabel('Redshift')
plt.ylabel('Parámetro de Hubble')
plt.legend()
plt.show()